...
/**
* Creates a return annotation for a complex number.
*
* @private
* @param {*} actual - actual return value
* @param {Object} opts - function options
* @returns {string} return annotation for complex number
*/
function complexAnnotation( actual, opts ) {
	var copts;
	var out;

	copts = objectAssign( {}, opts );
	copts.decimal = true;

	out = '<'+actual.constructor.name+'>';
	out += '[ '+numberAnnotation( real( actual ), copts );
	out += ', '+numberAnnotation( imag( actual ), copts );
	out += ' ]';
...
function genericAnnotation( actual, opts ) {
	...
	if ( isTypedArray( actual ) || isComplexTypedArray( actual ) || isBooleanArray( actual ) ) {
		...
		if ( isComplexTypedArray( actual ) ) {
			...
			f = genericAnnotation;
		} else if ( isBigInt64Array( actual ) || isBigUint64Array( actual ) ) {
			f = genericAnnotation;
		}
		out += '[ ';
		out += f( actual[ 0 ], opts );
